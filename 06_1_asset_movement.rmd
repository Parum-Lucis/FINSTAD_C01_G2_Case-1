...
# 2. Consolidate and pivot cleanly
...
# 3. Combine matrices and generate clean HTML-formatted facet labels
plotting_matrix_df <- bind_rows(cas_scaling_df, cas_lines_df) %>%
  ...
  mutate(
    # Shortened equation string
    Hurst_Eq = paste0("Var = ", round(Base_Var, 4), " * T^", round(2 * Hurst_Est, 2)),
    # HTML <br> handles line breaks cleanly inside ggplotly without stacking
    Asset_Facet = paste0("<b>", Asset, "</b> (H = ", round(Hurst_Est, 3), ")<br>", Hurst_Eq)
  )
...
for (current_space in unique(plotting_matrix_df$Space_Label)) {
  ...
  p_cas_true <- ggplot(space_data, 
    ...
    geom_point(aes(text = Tooltip_Label), size = 1.0, alpha = 0.5) +
    facet_wrap(~Asset_Facet, scales = "free_y") +
    scale_color_manual(values = c(
      "Empirical (Actual CAS Dynamics)" = "darkblue", 
      "Brownian Motion Benchmark (Linear Scaling)" = "darkgrey",
      "CAS Fractional Target (T^2H)" = "darkorange"
    )) +
    scale_linetype_manual(values = c(
      "Empirical (Actual CAS Dynamics)" = "solid", 
      "Brownian Motion Benchmark (Linear Scaling)" = "dashed",
      "CAS Fractional Target (T^2H)" = "dotdash"
    )) +
    theme_minimal(base_size = 9) +
    labs(
      title = paste("Fractional Scaling Properties:", current_space),
      x = "Time Horizon Scaling (Days, T)",
      y = "Aggregated Variance",
      color = "Model Hypothesis",
      linetype = "Model Hypothesis"
    ) +
    theme(
  ...
  # Adjust margins so the title and facet headers do not overlap
  plotly_plots_2[[current_space]] <- ggplotly(p_cas_true, height = 650) %>%
    layout(
